...
public class CustomMapPerformanceTest {
    ...
    public static void main(String[] args) {
        try (FileWriter writer = new FileWriter("custom_map_performance.csv")) {
            ...
            for (int size : SIZES) {
                ...
                start = System.nanoTime();
                for (int i = 0; i < size; i++) {
                    map.replace(keys.get(i), values.get(i), values.get(i)); // replacing with same value
                }
            ...
            System.out.println("Benchmark results saved to custom_map_performance.csv");
